...
import {
  ...
  borrarAuto,
  busquedaAuto,
} from "../server/Autos/services/autos.js";

import { obtenerPersonas } from "../server/Personas/services/personas.js";
      ...
      describe("al intentar agregar una nueva", () => {
        describe("si los datos válidos", () => {
          it("la crea, la guarda, y la devuelve", async () => {
            ...
            const { data: personaAgregada, status } = await axios.post(
              urlPersonas,
              datosPersona
            );
            ...
            const personaAgregadaEsperada = {
              ...datosPersona,
              id: personaAgregada.id,
            };
            assert.deepStrictEqual(
              personasDespues,
              personasAntes.concat(personaAgregadaEsperada)
            );
          });
        });
    ...
    describe("Autos", () => {
      ...
      describe("..", () => {
        it("devuelve un array con autos relacionadas con esa marca", async () => {
          ...
          let autosObtenidos;
          const { data, status } = await axios.get(urlAutos, {
            params: { tema: ".." },
          });
          assert.strictEqual(status, 200);
          autosObtenidos = data;
      ...
      describe("al mandarle datos válidos de un auto", () => {
        it("crea y agrega ese auto a las demas existentes", async () => {
          ...
          const { data: autoAgregado, status } = await axios.post(
            urlAutos,
            auto
          );
          assert.strictEqual(status, 201);
          ...
          assert.deepStrictEqual(
            autosDespues,
            autosAntes.concat(autoAgregadoEsperado)
          );
        });
      });
      ...
      describe("al mandarle un auto mal formateado", () => {
        it("no agrega nada y devuelve un error", async () => {
          ...
          await assert.rejects(axios.post(urlAutos, auto), (error) => {
            assert.strictEqual(error.response.status, 400);
            return true;
          });
